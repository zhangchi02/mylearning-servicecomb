...
import mylearning.servicecomb2_1.common.Animal;
import mylearning.servicecomb2_1.common.DeductFreeResourceRsp;
import mylearning.servicecomb2_1.common.Dog;
...
import mylearning.servicecomb2_1.intf.IParamService;
...
//  Schema not exist, microserviceName=provider, schemaId=, consumer interface=mylearning.servicecomb2.service.ICommonService; new producer not running or not deployed.
  ...
  public String test(@QueryParam("test") String test) throws Exception {
    ...
    if ("hihihi".equalsIgnoreCase(test)) {
      response = restTemplate.getForEntity("cse://mylearning-servicecomb:provider/provider/v0/hihihi?name=hihihi", String.class);
  ...
  public DateVO date(@QueryParam("test") String test) {
    ...
    if (test.equalsIgnoreCase("date2")) {
      result = restTemplate.getForObject("cse://provider/provider/v0/date2?str={1}&date={2}", DateVO.class, "test",
          "2021-01-22 00:00:00");
      return result;
    }
