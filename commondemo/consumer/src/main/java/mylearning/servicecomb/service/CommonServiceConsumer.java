...
import mylearning.servicecomb.intf.IParamService;
...
//  Schema not exist, microserviceName=provider, schemaId=, consumer interface=mylearning.servicecomb.service.ICommonService; new producer not running or not deployed.
  ...
  @RpcReference(microserviceName = "provider", schemaId = "paramService")
  ...
  @Path("/hello")
  @GET
  public String hello(@QueryParam("name") String name, @QueryParam("num") Integer num) {
    LOGGER.info("--------");
    if (helloDelay.get() > 0) {
      try {
  ...
  public String queryTeacher(Teacher teacher) {
    ...
    if (teacher.getName().equalsIgnoreCase("restTemplate")) {
      ...
      body.put("age", 28);
      String url = "cse://provider/provider/v0/queryTeacher?startTime=2021-01-21 00:00:00";
...
//    findDog sort: 5992104113028738701,weight: 5.9921041130287391E18,age: 599210411,price: 5992104113028738701.0,.
  ...
  public String listPerson() {
    ...
    Person person1 = new Person("aaa", "male");
    Person person2 = new Person("bbb", "male");
    list.add(person1);
    list.add(person2);
    return commonService.listPerson(list);
  }

  @Path("/asProvider")
  @GET
  public String asProvider(@QueryParam("name") String name) {
    return "consumer return success, " + name;
  }

  @Path("/hi")
  @GET
  public String hi() {
    return paramService.sayHi("name2");
  }

  @Path("/haha")
  @GET
  public String haha(@QueryParam("name") String name) {
    String url = String.format("cse://provider/provider/v0/haha?name=%s", name);
    String result = restTemplate.getForEntity(url, String.class).getBody();
    return result;
  }
}
